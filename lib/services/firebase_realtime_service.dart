...
class FirebaseRealtimeService {
  ...
  void _initDatabase() {
    try {
      ...
      connectedRef.onValue.listen((event) {
        ...
        debugPrint(connected ? '📶 Realtime DB connected' : '📵 Realtime DB disconnected');
  ...
  Future<List<Post>> getAllPosts() async {
    try {
      ...
      if (snapshot.exists) {
        final Map<dynamic, dynamic> postsMap = snapshot.value as Map<dynamic, dynamic>;
        final List<Post> posts = [];
        
        postsMap.forEach((key, value) {
          // Parse the post data
          final post = _parsePostData(key.toString(), value as Map<dynamic, dynamic>);
          posts.add(post);
        });
  ...
  Future<bool> likeReply(String replyId) async {
    try {
      ...
      if (replySnapshot.exists) {
        ...
        // Update likes count
        await _repliesRef.child(replyId).update({
          'likes': currentLikes + 1,
        });
  ...
  Future<List<Reply>> getRepliesForPost(String postId) async {
    try {
      ...
      if (snapshot.exists) {
        final Map<dynamic, dynamic> repliesMap = snapshot.value as Map<dynamic, dynamic>;
        final List<Reply> replies = [];
        
        repliesMap.forEach((key, value) {
          final reply = _parseReplyData(key.toString(), value as Map<dynamic, dynamic>);
          replies.add(reply);
        });
  ...
  Future<void> createSamplePosts() async {
    try {
      ...
      final snapshot = await _postsRef.get();
      if (snapshot.exists) {
        debugPrint('Posts already exist in Realtime Database, skipping sample creation');
        return;
      }
