...
class FirebaseService {
  ...
  Future<void> saveMahoroConversation(String userId, Map<String, dynamic> conversationData) async {
    try {
      String convoId = conversationData['id'];
      await _db.collection('users').doc(userId)
          .collection('mahoro_conversations').doc(convoId).set(conversationData);
  ...
  Future<void> saveMugangaSubscription(String userId, Map<String, dynamic> subscriptionData) async {
    try {
      await _db.collection('users').doc(userId)
          .collection('muganga_subscriptions').doc('current').set(subscriptionData);
  ...
  Future<void> saveUserSettings(String userId, Map<String, dynamic> settingsData) async {
    try {
      await _db.collection('users').doc(userId).collection('settings').doc('user_settings').set(settingsData);
  ...
  Future<void> saveCompleteUserDataStructure(Map<String, dynamic> completeUserData) async {
    try {
      ...
      for (var userData in usersList) {
        ...
        if (user.containsKey('mahoro_conversations')) {
          ...
          for (var conversation in conversations) {
            await saveMahoroConversation(userId, conversation as Map<String, dynamic>);
          }
        }
        
        // Save Muganga subscription
        if (user.containsKey('muganga_subscription')) {
          await saveMugangaSubscription(userId, user['muganga_subscription'] as Map<String, dynamic>);
        }
        
        // Save user settings
        if (user.containsKey('settings')) {
          await saveUserSettings(userId, user['settings'] as Map<String, dynamic>);
        }
      }
